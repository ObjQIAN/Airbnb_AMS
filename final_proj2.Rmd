...
data_to_train <- nhoods_sub %>%
  select(neighbourhood,price, accommodates, AvrVal,bathrooms,bedrooms,beds,cleaning_fee,high_income_pct,greenroof_nn3,low_income_pct,market_nn3,median_HH_num,owned_pct,pct_dutch_bkg,pct_nonwes_bkg,pct_wes_bkg,reviews_per_month,tram_nn3,property_type,room_type)
 
inTrain <- createDataPartition(
              y = paste(data_to_train$property_type,data_to_train$room_type), 
              p = .70, list = FALSE)
ams.training <- data_to_train[inTrain,] %>% st_drop_geometry()
...
reg.training <-
   lm(price ~ ., data = as.data.frame(ams.training)%>%select(-neighbourhood))
