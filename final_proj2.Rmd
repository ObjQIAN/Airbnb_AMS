...
nhoods <- left_join(nhoods,sentiment_list, by = 'id')
...
ggplot() +
  geom_sf(data = district, fill = NA)+
  geom_hex(data = nhoods, aes(x = st_coordinates(nhoods)[,1], y = st_coordinates(nhoods)[,2], z = price),
           stat = "summary", 
           fun = mean,
           binwidth = c(300,300)) +
  #coord_fixed() +  
  labs(title = "Listing density Hexbin map in Amsterdam",
       ...
       x = "longitude",
       y = "latitude") +
  scale_fill_viridis(name = "Mean Price", option = "C") +
  theme_light()
```
...
listing_details <- listing_details %>%
  select(-amenities,-host_verifications,-host_since) %>%
  filter(minimum_nights<32 & bathroom < 11) %>%
  na.omit()
...
nhoods_sub <- nhoods%>%
  select(id,price, neighbourhood, postcode4,number_of_reviews,calculated_host_listings_count,AvrVal,age,tram_nn3,polarity_name,polarity_overview,polarity_rules,review_polarity, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,daycare_num,fire_station,dis_to_sub,dis_to_train,elec_using,zip_house_value,gas_using,not_ocp_home,owned_pct,rental_corp,rental_home,median_HH_num,high_income_pct,low_income_pct,avr_fam_size,pct_nonwes_bkg,pct_wes_bkg,pct_dutch_bkg,famale_residents,male_residents,residents) %>%st_drop_geometry() 
...
nhoods_sub <- inner_join(listing_details, nhoods_sub, by = 'id')%>% filter(price < 300)
...
data_to_train <- nhoods_sub %>%
  select(price, accommodates, AvrVal,bathrooms,bedrooms,beds,cleaning_fee,high_income_pct,greenroof_nn3,low_income_pct,market_nn3,median_HH_num,owned_pct,pct_dutch_bkg,pct_nonwes_bkg,pct_wes_bkg,reviews_per_month,tram_nn3,room_type,polarity_name,polarity_overview,polarity_rules,review_polarity)
...
## model, neighborhood model, lagmodel, lagsentiment model
