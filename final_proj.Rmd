...
nhoods_sub <- nhoods%>%
  select(id,price, postcode4,number_of_reviews,calculated_host_listings_count,AvrVal,highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,) %>%st_drop_geometry() 
...
inTrain <- createDataPartition(
              y = paste(nhoods_sub$property_type,
                        nhoods_sub$room_type,
                        nhoods_sub$cancellation_policy,
                        nhoods_sub$bed_type,
                        nhoods_sub$host_has_profile_pic), 
              p = .70, list = FALSE)
...
reg.training <- 
  lm(price ~ ., data = as.data.frame(ams.training) %>% 
                             dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,property_type,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths))
#extra_people,host_since,
ams.test <-
  ams.test %>%  dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,property_type,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths)%>%
  na.omit() 
...
```{r}
reg1 <-  lm(price ~ ., data = as.data.frame(nhoods) %>% 
                             dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct ))
summary(reg1)
```
