...
nhoods_sub <- nhoods%>%
  select(id,price, neighbourhood, postcode4,number_of_reviews,calculated_host_listings_count,AvrVal,age, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,daycare_num,fire_station,dis_to_sub,dis_to_train,elec_using,zip_house_value,gas_using,not_ocp_home,owned_pct,rental_corp,rental_home,median_HH_num,high_income_pct,low_income_pct,avr_fam_size,pct_nonwes_bkg,pct_wes_bkg,pct_dutch_bkg,famale_residents,male_residents,residents) %>%st_drop_geometry() 
...
ams.training <- ams.training%>%
  dplyr::select(price, postcode4, geometry,highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal  ) 

ams.test <- ams.test%>%
  dplyr::select(price, postcode4, geometry,highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal ) 
reg.training <- 
  lm(price ~ ., data = as.data.frame(ams.training) %>% 
                             dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal,host_id,host_since,host_response_time,host_response_rate,host_listings_count,host_verifications,host_has_profile_pic,is_location_exact,property_type,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,amenities,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month ))
...
reg.training <-
   lm(price ~ ., data = as.data.frame(ams.training) %>% 
                             dplyr::select(price,market_nn3,swimming_nn3 ,fire_station  , not_ocp_home   ,rental_home,median_HH_num,
famale_residents ,pct_dutch_bkg  ,male_residents  ,residents ,AvrVal  ,host_id  ,room_type,accommodates ,
bathrooms ,bedrooms  ,cleaning_fee,minimum_nights,cancellation_policy,review_scores_rating,reviews_per_month,amenities_lengths))
...
reg.training2 <- 
  lm(price ~ ., data = as.data.frame(ams.training) %>% 
                             dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,daycare_num,fire_station,not_ocp_home,owned_pct,rental_corp,rental_home,median_HH_num,high_income_pct,low_income_pct,avr_fam_size,pct_nonwes_bkg,pct_wes_bkg,pct_dutch_bkg,famale_residents,male_residents,residents,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths))
#extra_people,host_since,
#dis_to_sub,dis_to_train,elec_using,zip_house_value,gas_using,beds,bed_type,postcode4,property_type,

reg2 <- 
  lm(price ~ ., data = as.data.frame(ams.training) %>% 
                             dplyr::select(price,neighbourhood, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,pct_dutch_bkg,pct_nonwes_bkg,avr_fam_size,low_income_pct,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,property_type,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths))


ams.test <-
  ams.test %>%  dplyr::select(price,neighbourhood, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,daycare_num,fire_station,dis_to_sub,dis_to_train,elec_using,zip_house_value,gas_using,not_ocp_home,owned_pct,rental_corp,rental_home,median_HH_num,high_income_pct,low_income_pct,avr_fam_size,pct_nonwes_bkg,pct_wes_bkg,pct_dutch_bkg,famale_residents,male_residents,residents,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,property_type,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths)%>%
  na.omit() 
...
```
```{r}
xxx <- ams.training%>%dplyr::select(price, highrise_nn3,greenroof_nn3,wallart_nn3,market_nn3,swimming_nn3,daycare_num,fire_station,not_ocp_home,owned_pct,rental_corp,rental_home,median_HH,high_income_pct,low_income_pct,avr_fam_size,pct_nonwes_bkg,pct_wes_bkg,pct_dutch_bkg,famale_residents,male_residents,residents,AvrVal,host_id,host_response_time,host_response_rate,host_listings_count,host_has_profile_pic,is_location_exact,room_type,accommodates,bathrooms,bedrooms,beds,bed_type,cleaning_fee,extra_people,minimum_nights,maximum_nights,cancellation_policy,review_scores_rating,reviews_per_month,host_since_days,host_veri_length,amenities_lengths)%>% st_drop_geometry()%>% na.omit()
reg.rf <- randomForest(price ~ ., data = xxx, ntree = 500, mtry = 3)
...
ams_census <- 
  st_read("https://raw.githubusercontent.com/ObjQIAN/Airbnb_AMS/main/data/ams_census.geojson") %>%
   st_transform('EPSG:7415') %>%
  dplyr::select(postcode4,geometry)
...
There exists a robust positive correlation between MedRent, MedHHIn, and pctWhite. Conversely, there is a notable negative correlation between pctPoverty and MedRent, MedHHInc, as well as pctWhite.
...
Then we use Pearson’s R to initially judge the correlation and degree of explanation between features and sale_price.

As can be seen from the figure below, sale_price generally increases with the increase of total_livable_area, and there is a significant positive correlation (R-square = 0.33, p<2.2e-16).
...
We performed regression analysis on internal features, external features and spatial features and sale_price. In Table 4.1 Summary Statistics of Model, we can see that these features can jointly explain 94% of sale_price (p<2.2e-16). Among them, the relationship between total_livable_area, garage_spaces, exterior_condition, landmark.Buffer, market_nn3, TotalPop, MedHHInc, pctBachelors and pctPovertyy and sale_price is very significant.
...
The following is Table Summary of Model Evaluation Parameters.
...
We selected the feature with strong explanatory power for sale_price to explore its boundary influence. As you can see from the figure below, our model performs better when total_livable_area is in the 500-2000 range.
...
Spatial Lag of Price is obviously positively related to Sale price. Spatial Lag of Error is relatively gentle.
...
Then we did Moran's I. From the figure below, Moran’s I deviates far from 0.0, indicating that there is an obvious spatial lag.
...
Here's how our estimated average house prices compare to actual average house prices by region. The two are generally close, but there are still certain deviations.
